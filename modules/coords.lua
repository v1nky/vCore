...
function vCore:MapCoords()
    ...
    local vCore_CoordsFrame = CreateFrame("Frame", "vCore_Coords", WorldMapFrame, "vCore_Coords")

    vCore_CoordsFrame:SetScript("OnLoad", function(self)
        self:RegisterEvent("PLAYER_ENTERING_WORLD")
        self:SetPoint(unpack(cfg.location))
    end)

    vCore_CoordsFrame:SetScript("OnUpdate", function(self, elapsed)
        if IsInInstance() or not vCoreDB.MapCoords  then
